...
class APESCriticNet(nn.Module):
    ...
    def forward(self, x, s, g, coefficients):
        ...
        x = self.LeakyReLU(x)
        x = self.mp(x)
        print("x1size", x.shape)
        x = self.conv2(x)
        x = self.LeakyReLU(x)
        x = self.mp(x)
        print("x2size", x.shape)
        x = self.conv3(x)
        x = self.LeakyReLU(x)
        x = self.mp(x)
        print("x3size", x.shape)
        x = torch.flatten(x)
        print("x4size", x.shape)
        s = torch.flatten(s)
        print("x5size", s.shape)
        g = torch.flatten(g)
        print("x6size", g.shape)
...
class APESGeneratorNet(nn.Module):
    ...
    def rsample(self, x, s, g):
        dist = Dirichlet(self.forward(x, s, g))
        dirchlet_sample = dist.rsample()
        print("dirch sample", dirchlet_sample)
